...
const TRANSLATIONS={
  en:{
    ...
    'queue.title':'Account Queue','queue.description':'This order applies to new sessions and account failover. Existing sessions keep their available account when rankings change.','queue.onlyAvailable':'Show only available accounts','queue.noAvailable':'No available accounts match this filter.','queue.aria':'Account cards','metrics.nextAccount':'Next allocation','metrics.monthlyMode':'Monthly','metrics.lastSelected':'Last selected',
  ...
  'zh-CN':{
    ...
    'queue.title':'账号队列','queue.description':'此顺序用于新会话和需要换号的请求。已有绑定且账号可用时继续使用原账号，不受排名变化影响。','queue.onlyAvailable':'只显示可用账号','queue.noAvailable':'没有符合条件的可用账号。','queue.aria':'账号卡片','metrics.nextAccount':'新分配首选','metrics.monthlyMode':'Monthly','metrics.lastSelected':'最近选择',
