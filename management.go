...
func SettingsFromConfig(cfg Config) SettingsPayload {
	return SettingsPayload{
		AffinityMode:                    cfg.AffinityMode,
		AffinityTTL:                     cfg.AffinityTTL.String(),
...
func ConfigFromSettings(base Config, payload SettingsPayload) (Config, error) {
	...
	if err := applyAffinityConfig(&cfg, payload.AffinityMode, payload.AffinityTTL); err != nil {
		return Config{}, err
	}
...
<label class="field"><span data-i18n="settings.affinityMode">账号绑定方式</span><select id="affinityMode"><option value="fill-first" data-i18n="settings.affinityFillFirst">保留可用账号</option><option value="off" data-i18n="settings.affinityOff">每次按排名选择</option></select></label>
<label class="field"><span data-i18n="settings.affinityTTL">绑定有效期</span><input id="affinityTTL" placeholder="4h"></label>
...
const TRANSLATIONS={
  en:{
    ...
    'queue.title':'Account Queue','queue.description':'This order applies to new allocations and account failover. With account binding enabled, usable bindings survive ranking changes.','queue.onlyAvailable':'Show only available accounts','queue.noAvailable':'No available accounts match this filter.','queue.aria':'Account cards','metrics.nextAccount':'Next allocation','metrics.monthlyMode':'Monthly','metrics.lastSelected':'Last selected',
  ...
  'zh-CN':{
    ...
    'queue.title':'账号队列','queue.description':'此顺序用于新分配和需要换号的请求。开启账号绑定时，可用的绑定账号不受排名变化影响。','queue.onlyAvailable':'只显示可用账号','queue.noAvailable':'没有符合条件的可用账号。','queue.aria':'账号卡片','metrics.nextAccount':'新分配首选','metrics.monthlyMode':'Monthly','metrics.lastSelected':'最近选择',
...
const INLINE_TRANSLATIONS=[
  ...
  ['刷新额度','Refresh Quota'],['编辑','Edit'],['请重新登录','Please re-login'],['不可用','Unavailable'],['最后错误','Last error'],['下次重试','Next retry'],['长额度','Long quota'],[' 重置：',' reset: '],['Monthly：','Monthly: '],['调度：','Scheduler: '],['新分配首选：','Next allocation: '],['最近选择：','Last selected: '],
